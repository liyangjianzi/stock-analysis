...
- **Two independent scores.** Fundamental score is **0–6** (one point per threshold passed). Technical score is **0–5** (price>EMA50, RSI in 35–70, recent bullish MACD crossover, positive regression-channel slope, volume confirmation). `generate_signals` fuses them as `0.70*(fund/6) + 0.30*(tech/5)` → Buy ≥0.60, Hold ≥0.40, else Watch. Posture maps tech ≥3 Bullish / 1–2 Neutral / 0 Bearish. Changing the max of either score requires updating these divisors.
